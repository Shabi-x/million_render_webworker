...
const DataList = () => {
  ...
  // 获取数据映射
  const getDataMap = useCallback((newData) => {
  ...
  const makeWorker = useCallback((calcInfo) => {
    ...
    worker.onmessage = (e) => {
      ...
      setFooterData(prev => {
        ...
        const existIndex = newFooterData.findIndex(data => data[0] === e.data[0]);
  ...
  const handleCheckboxChange = useCallback((selectedTypes) => {
    ...
    if (newType) {
      ...
      // 移除统计类型
      setFooterData((prev) => 
        prev.filter((item) => item[0] !== removedType)
      );
    }
  ...
  return (
    <div className="data-list">
      ...
      <div className="table-list" style={{ height: 'calc(100vh - 200px)' }}>
        <ReactDataGrid
          columns={columns}
          rows={tableData}
          rowHeight={35}
